...
import {
    BarretenbergApiAsync, BarretenbergApiSync,
    Crs,
...
export class NoirNode {
    ...
    async init(circuit: any) {
        ...
        this.api = await newBarretenbergApiSync();

        const [exact, total, subgroup] = this.api.acirGetCircuitSizes(
            this.acirBufferUncompressed,
        );
        const subgroupSize = Math.pow(2, Math.ceil(Math.log2(total)));
        const crs = await Crs.new(subgroupSize + 1);
        this.api.commonInitSlabAllocator(subgroupSize);
        this.api.srsInitSrs(
        ...
        this.acirComposer = this.api.acirNewAcirComposer(subgroupSize);
    }
    ...
    async generateProof(witness: Uint8Array) {
        const proof = this.api.acirCreateProof(
            this.acirComposer,
            this.acirBufferUncompressed,
    ...
    async verifyProof(proof: Uint8Array) {
        this.api.acirInitProvingKey(this.acirComposer, this.acirBufferUncompressed);
        const verified = this.api.acirVerifyProof(this.acirComposer, proof, false);
        return verified;
    }
